...
const REGEX_ACTION_DEFINITION = /PRM_ACTIONS_FOR_REDUCER_NAME\s+=\s+['"]([^'"]*)['"]/; // used to find an action file related to a reducer
...
const COMMAND_LINE_ARGS = [
  ...
  {
    name: 'coreReducerFilenames',
    alias: 'f',
    type: String,
    typeLabel: '[underline]{filename1.js},[underline]{filename2.js},...',
    description: 'if any of these comma-delimited filenames are imported from a folder containing a reducer, then the reducer will be considered to be in use.  If this is not specified, then any action file must contain a [italic]PRM_ACTIONS_FOR_REDUCER_NAME definition to be found.',
  },
  {
    name: 'disableCache',
    alias: 'd',
  ...
  {
    name: 'globalImportsOutputPath',
    alias: 'g',
    type: String,
    typeLabel: '[underline]{path}',
    description: 'the output path for the globalReducerImports.js file.',
    mandatory: true,
  },
  {
    name: 'showHelp',
  ...
  },
  {
    ...
    type: String,
    typeLabel: '[underline]{path1},[underline]{path2},...',
    description: '(optional) the root path or paths under which all the redux reducers can be found - this may be the same as component path(s)',
    mandatory: true,
  },
  {
    name: 'sagaFilename',
    ...
    description: 'if you use sagas, this is the filename where all sagas will be contained (e.g., sagas.js).',
  },
  {
    name: 'verboseLogging',
...
class HMR_ReduxMapper {
  ...
  _findReducerDefinitions(folder) {
    ...
    this._execOnAllFilesRecursive(folder, fileCur => {
      ...
      if (nameMatch && nameMatch.length > 1) {
        ...
        const sagaPathMunged = this._opts.sagaFilename && sagaPath.replace(this._getBasePath(path.dirname(this._opts.reducerMapOutputPath)), '.');
        reducers.push({
          reducerName,
  ...
  _exportGlobalModulesFile() {
    const outputFolder = path.dirname(this._opts.globalImportsOutputPath) + path.sep;
    ...
    fs.writeFileSync(this._getBasePath(this._opts.globalImportsOutputPath),
      '/* AUTOGENERATED FILE - DO NOT MODIFY */\r\n' +
      '/* generated by HMR_ReduxMapper */\r\n' +
      imports + "\r\nexport default {\r\n" + exports + "};\r\n");
  }
    ...
    const sections = [
      {
        ...
        content: 'This is a module which generates a [italic]{global} and [italic]{component specific} mapping file ' +
          ...
          'constant which this tool will look for.',
      },
      {
        header: 'Options',
...
//
// actually initialize and start the tests here
//
